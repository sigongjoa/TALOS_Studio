...
# Create directory for the new content
mkdir -p ./gh-pages/$SHORT_SHA

# Copy visualization files
cp -r ./output_visualizations/* ./gh-pages/$SHORT_SHA/

# Write top part of the new index.html
...
# Generate the list of deployments
find ./gh-pages -mindepth 1 -maxdepth 1 -type d | sort -r | while read -r dir; do
  COMMIT_HASH=$(basename "$dir")
  # Only process if it looks like a short commit hash
  ...
  <div>
  <a class="font-mono text-lg text-primary hover:underline" href="./$COMMIT_HASH/">$COMMIT_HASH</a>
...
# Write bottom part of the new index.html
cat <<'EOF' >> ./gh-pages/index.html
</ul>
</div>
<div class="mt-12">
<h2 class="text-3xl font-bold font-display mb-6">Latest Visualization Result</h2>
<div class="bg-card-light dark:bg-card-dark p-4 sm:p-6 rounded-lg shadow-md border border-border-light dark:border-border-dark">
<p class="text-center text-subtle-light dark:text-subtle-dark">(This section will be implemented in a future update)</p>
</div>
</div>
</main>
</div>

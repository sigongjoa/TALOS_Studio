...
# Define the model architecture (copied from model_torch.py)
...
def run_manga_line_extraction_inference(input_image_path, output_image_path):
    ...
    weights_path = os.path.join(script_dir, 'libs', 'MangaLineExtraction_PyTorch', 'erika.pth')
    ...
    import time
    with torch.no_grad():
        print(f"[DEBUG] MangaLineExtraction: Attempting to read image from: {input_image_path}")
        src = cv2.imread(input_image_path, cv2.IMREAD_GRAYSCALE)
        
        if src is None:
            print(f"[ERROR] MangaLineExtraction: cv2.imread returned None for {input_image_path}. Check file path and integrity.")
            return # Exit if image could not be loaded

        print(f"[DEBUG] MangaLineExtraction: Image successfully loaded. Type: {type(src)}, Shape: {src.shape}")
        ...
        yc_resized = yc[0:src.shape[0],0:src.shape[1]]
        cv2.imwrite(output_image_path, yc_resized)

if __name__ == "__main__":
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))
    DEFAULT_INPUT_IMAGE = os.path.join(BASE_DIR, "input", "test_image.jpg")
    DEFAULT_OUTPUT_DIR = os.path.join(BASE_DIR, "output")
    DEFAULT_OUTPUT_IMAGE = os.path.join(DEFAULT_OUTPUT_DIR, "manga_line_extraction_detection.png")
    os.makedirs(DEFAULT_OUTPUT_DIR, exist_ok=True)

    run_manga_line_extraction_inference(DEFAULT_INPUT_IMAGE, DEFAULT_OUTPUT_IMAGE)
    print("\nMangaLineExtraction script finished.")
